...
const AMIEAnalysis: React.FC = () => {
  ...
  const modalities = [
    ...
    { id: 'mri', name: 'Brain MRI', icon: Brain, description: 'AI Analysis', realAI: false },
    { id: 'ct', name: 'CT Scan', icon: Scan, description: 'AI Analysis', realAI: false },
    { id: 'pet', name: 'PET Scan', icon: Activity, description: 'AI Analysis', realAI: false },
  ];
  ...
  const handleAnalyze = async () => {
    ...
    // Fallback: Demo AI analysis for non-X-ray modalities
      ...
      <div className="bg-gradient-to-r from-blue-900/30 to-purple-900/30 border border-white/10 rounded-2xl p-6 mb-6">
        <div className="flex items-center gap-4 mb-2">
          ...
          <div>
            ...
            <p className="text-gray-400 text-sm">
              Predicted using TorchXRayVision
            </p>
          </div>
        </div>
        ...
        <div className="mt-4 p-3 bg-yellow-900/20 border border-yellow-500/20 rounded-xl flex items-start gap-3">
          ...
          <p className="text-yellow-400/80 text-xs">
            <strong>Research Prototype:</strong> Not for clinical diagnosis. Always consult qualified healthcare professionals for medical decisions.
          </p>
        </div>
      </div>
        ...
        <div className="lg:col-span-1 space-y-4">
              ...
              {modalities.map(mod => {
                ...
                return (
                  <button
                    ...
                    className={`w-full p-3 rounded-xl text-left transition-all ${selectedModality === mod.id
          ...
          >
            ...
            <label className="block cursor-pointer">
              <div className={`border-2 border-dashed rounded-xl p-6 text-center transition-colors ${selectedImage ? 'border-green-500/30 bg-green-900/10' : 'border-white/10 hover:border-white/20'
                }`}>
                {selectedImage ? (
                  <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    {apiResult.findings.map((finding, idx) => (
                      <div key={idx} className="flex items-center justify-between p-2 bg-black/30 rounded-lg">
                        ...
                        <div className="flex items-center gap-2">
                          <span className={`text-xs px-2 py-0.5 rounded ${finding.severity === 'High' ? 'bg-red-500/20 text-red-400' :
